async function drawHistogram() {
  ...
  // Create a function to draw bars so we can reuse it for different metrics
  // metric is a string that corresponds to a key in the data
  const drawBars = (metric) => {
    ...
    const xAxisLabel = xAxis
      ...
      .attr("y", 40)
      .text(metric.replaceAll("_", " ").toUpperCase())
  ...
  const metrics = ["dew_point", "humidity", "cloud_cover"]; // list of metrics to draw histograms for

  metrics.forEach(drawBars); // draw histogram for each metric
}
drawHistogram();
